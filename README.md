...
| **DB** | H2 (파일 DB, 개발용) |
| **대기열** | Redis (많은 사람이 동시 신청 시 선착순 요청 큐, 0.5초마다 순서대로 처리) |
...
- **학생**: 강의 목록·필터·신청, **선착순 요청 대기열**(동시 신청 시 큐에 쌓여 순서대로 처리), 대기 순번 조회·포기, 내 수강 시간표, 신청 취소, 마이페이지
